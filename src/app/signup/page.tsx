...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    // TODO: Create account with Supabase
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                s <= step ? 'bg-gold text-navy' : 'bg-gray-200 text-gray-500'
              }`}>
                {s}
              </div>
              {s < 3 && <div className={`w-12 h-0.5 ${s < step ? 'bg-gold' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {step === 3 && (
              <>
                  ...
                  <div className="flex gap-4">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const langs = e.target.checked 
                            ...
                            : formData.languages.filter(l => l !== 'en')
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          const langs = e.target.checked 
                            ? [...formData.languages, 'es']
                            : formData.languages.filter(l => l !== 'es')
                          updateField('languages', langs)
                        }}
                        className="rounded border-gray-300"
                      />
                ...
                <div>
                  <label className="block text-sm font-medium text-navy mb-2">Send Time (Monday)</label>
                  <input
                    type="time"
